...
$positions = [
    'heizungsart' => [100, 97, 110, 7],
    'hersteller' => [50, 150, 80, 7],
    'baujahr' => [120, 180, 70, 7],
    'heiztechnik' => [60, 175, 80, 7],
    'efh' => [120, 176, 5, 5],
    'energietraeger' => [150, 180, 80, 7],
    'zentralheizung' => [160, 190, 5, 5],
    'angaben_von' => [50, 195, 80, 7],
    'energieausweis_datum' => [120, 196, 70, 7],
...
foreach ($positions as $key => $pos) {
    if ($key === 'efh' || $key === 'zentralheizung') {
        // Checkbox: Fülle ein Rechteck, falls das Feld true ist
        if ($data[$key]) {
            drawFilledRectangle($pdf, $pos[0], $pos[1], $pos[2], $pos[3]);
        }
    } else {
        // Text in einem bestimmten Bereich schreiben (MultiCell für Zeilenumbruch)
        $pdf->SetXY($pos[0], $pos[1]);
        $pdf->MultiCell($pos[2], 7, $data[$key], 0, 'L', false);  // Dynamischer Textumbruch
    }
}
