...
export async function POST(request: Request) {
  ...
  try {
    ...
    const pdfFile = files.find(file => file === 'Heizungsplakette_Ausgefuellt.pdf')
    let pdfPath: string | undefined

    if (pdfFile) {
      pdfPath = path.join(phpDir, pdfFile)
    ...
    const mailOptions: nodemailer.SendMailOptions = {
      ...
      text: `
        ...
        ${pdfPath ? '' : 'Hinweis: Die generierte PDF-Datei konnte nicht gefunden werden. Wir werden Ihnen diese separat zusenden.'}
